...
struct Position {
  ...
  static bool equal(const Position &p1, const Position &p2) {
    return std::equal(p1.piece + Origin, p1.piece + Origin + Stride * RankNum, p2.piece + Origin) && std::equal(p1.hand[Black], p1.hand[White], p2.hand[Black]) && p1.turn == p2.turn;
  }
...
struct sci::Micro686AI::Impl {
  ...
  static int constexpr kLimitSeconds = 30;
  size_t index;
  size_t doneMove;
